...
async function balance(obj: EthqlAccount, { unit } /* args */, { services: { ethService } }: EthqlContext) {
  const bal = await ethService.fetchBalance(obj);
  return unit ? Web3.utils.fromWei(bal, unit) : bal;
}

async function code(obj: EthqlAccount, args, { services: { ethService } }: EthqlContext): Promise<string> {
  return ethService.fetchCode(obj);
}

async function transactionCount(obj: EthqlAccount, args, { services: { ethService } }: EthqlContext): Promise<number> {
...
async function type(obj: EthqlAccount, args, { services: { ethService } }: EthqlContext): Promise<EthqlAccountType> {
