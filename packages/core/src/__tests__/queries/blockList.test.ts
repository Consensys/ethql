...
test('blockList: select multiple blocks by specific numbers', async () => {
  const query = `
    {
      blockList(numbers: [1202, 20502, 292]) {
        hash
      }
    }
  `;
...
test('blockList: select multiple blocks by specific hashes', async () => {
  ...
  const query = `
    {
      blockList(hashes: ${hashes}) {
        number
      }
    }
  `;

  const expected = { data: { blockList: [{ number: 1234 }, { number: 1235 }, { number: 12342 }] } };
  const result = await execQuery(query);
  expect(result).toEqual(expected);
});

test('blockList: error when selecting blocks with both numbers and hashes', async () => {
  const hashes = `["0x624d6c50f4edff05693806953b211050ef3e674ed18b1a1a6e64352086006f9e",
      "0xbfe0f792a89bd44e6c22224a84721edfedb334e521afb365fd397442bc1b2b81",
      "0x0cd6b3ef09f74b86fd8e17122deae11c1016a578797472bee1a3bb138323954b"]`;
  ...
  const query = `
    {
      blockList(hashes: ${hashes}, numbers: [1202, 20502, 292]) {
        number
      }
    }
  `;
...
test('blockList: error when selecting blocks with no numbers or hashes', async () => {
  const query = `
    {
      blockList {
        number
