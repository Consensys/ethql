...
export function initWeb3(config: Options): Web3 {
  ...
  return config.batching ? batchingProxy(web3) : web3;
}
 ...
 */
function batchingProxy(web3: Web3): Web3 {
  ...
  interface Web3EthCommand {
    // The name of the web3.eth function that was called.
    name: string;
    // A reference to the web3.eth function.
  ...
  const dataloader = new DataLoader<Web3EthCommand, any>(keys => {
